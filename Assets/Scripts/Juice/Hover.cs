...
public class Hover : MonoBehaviour {
	...
	void Start() {
		if (doLocal) { center = transform.localPosition; } else { center = transform.position; }
	}
	...
	void UpdateHover() {
		if (doLocal) { transform.localPosition = center; } else { transform.position = center; }

		for (int i = 0; i < oscis.Length; i++) {
			if (doLocal) { transform.localPosition += offsets[i] * oscis[i].Update(); } else { transform.position += offsets[i] * oscis[i].Update(); }
		}
	}
}
