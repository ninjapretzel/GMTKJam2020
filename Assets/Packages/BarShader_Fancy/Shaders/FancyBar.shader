...
Shader "Bar/Fancy" {
	Properties {
		// Toggles the bar from filling horizontally or vertically.
		[Toggle(VERTICAL)] _VERTICAL("Vertical Orientation", Float) = 0
		// Toggles the bar filling direction
		[Toggle(FLIP_DIRECTION)] _FLIP_DIRECTION("Flip Direction?", Float) = 1
	...
	SubShader {
		...
		Pass {
			CGPROGRAM
				...
				#pragma multi_compile __ FLIP_DIRECTION
				...
				fixed4 frag(v2f IN, UNITY_VPOS_TYPE screenPos : VPOS) : SV_Target {
					...
					#ifdef VERTICAL
						half4 barColor = tex2D(_MainTex, IN.texcoord.yx);
					#else
					...
					if (_Border > 0) { 
						#ifdef VERTICAL
							float dborderx = min(x, abs(1.0-x)) * _SizeInfo.x / 4.0;
							float dbordery = min(p, abs(1.0-p)) * _SizeInfo.y / 4.0;						
						#else
					...
					p += sin(
						...
						y
					#endif
					* w_freq + _WavePan * time + sin(time * w_speed) * w_wobble) * w_amp;
